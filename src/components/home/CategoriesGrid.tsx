...
export default function CategoriesGrid({
  ...
  return (
    <section className="container-qtech beach-section py-16 lg:py-20">
      ...
      <div className="mt-10 grid snap-x gap-4 sm:grid-cols-2 lg:grid-cols-4 sm:overflow-visible sm:snap-none">
        {categories.slice(0, 8).map((cat) => {
          ...
          return (
            <Link
              ...
              className="group relative flex min-w-[260px] snap-start flex-col overflow-hidden rounded-2xl beach-card p-0 transition hover:-translate-y-1 hover:shadow-xl sm:min-w-0"
            >
              ...
              <div className="relative h-36 w-full overflow-hidden">
                ...
                <span className="absolute left-3 top-3 flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-brand-500 to-coral-400 text-white shadow-md backdrop-blur">
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                <span className="mt-auto inline-flex items-center gap-1 pt-3 text-xs font-medium text-coral-600">
        ...
        <Link
          ...
          className="group flex min-w-[260px] snap-start flex-col items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed border-coral-300 bg-gradient-to-br from-coral-50 to-sand-50 p-0 transition hover:border-coral-500 hover:-translate-y-1 hover:shadow-lg sm:min-w-0"
        >
          <div className="flex h-full flex-col items-center justify-center gap-3 p-5 text-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-coral-600 shadow-sm">
            ...
            <div>
              <p className="text-sm font-semibold text-coral-700">{locale === 'zh' ? '浏览全部设备' : locale === 'ar' ? 'عرض جميع المعدات' : 'Browse All Machines'}</p>
              <p className="mt-0.5 text-xs text-coral-500">{categories.length} {t('home.categories.productCount')}</p>
            </div>
          </div>
        </Link>
      </div>
