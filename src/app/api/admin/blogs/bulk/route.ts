...
/** PATCH bulk action over selected blog posts: publish | unpublish | delete. */
export async function PATCH(req: NextRequest) {
  ...
  try {
    if (action === 'delete') {
      await prisma.blogPost.deleteMany({ where: { id: { in: ids as string[] } } });
    } else {
    ...
    console.error('[admin/blogs/bulk] PATCH failed:', err);
